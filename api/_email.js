...
function renderHtml({ firstName, sessionLabel, sessionTime, meetLink, requestedAudit }) {
  ...
  const auditBlock = requestedAudit
    ? `<div style="background:#F1F5FF;border:1px solid #DBE3FF;border-radius:14px;padding:18px 22px;margin:24px 0"><p style="margin:0;color:#1E3A8A;font-size:14px;line-height:1.6"><strong>Heads up:</strong> you ticked the free AI audit. We'll reach out within 1&ndash;2 business days to schedule it, with priority scheduling for mentioning the workshop.</p></div>`
    : '';
...
<head>
  ...
  <title>You're in, Nekter AI Workshop</title>
</head>
<body style="margin:0;padding:0;background:#F8FAFF;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#0F172A">
  <div style="max-width:560px;margin:0 auto;padding:40px 24px">
    <p style="font-size:12px;letter-spacing:.12em;text-transform:uppercase;color:#6C47FF;font-weight:700;margin:0 0 8px">Nekter AI &middot; Monthly Workshop</p>
    <h1 style="font-size:30px;font-weight:900;letter-spacing:-.02em;line-height:1.15;margin:0 0 18px">You're in, ${safeName}.</h1>
    <p style="font-size:16px;line-height:1.7;color:#334155;margin:0 0 22px">Thanks for registering for <strong>The 5 AI Skills Every Business Owner Should Actually Have</strong>. You're confirmed for ${safeSession}${safeTime ? ` at ${safeTime}` : ''}.</p>
    ...
    <ul style="font-size:15px;line-height:1.75;color:#334155;padding-left:20px;margin:0 0 24px">
      <li>Have a real business problem in mind. You'll build a custom GPT around it live.</li>
      <li>Free accounts for Claude and ChatGPT. That's all you need.</li>
...
function renderText({ firstName, sessionLabel, sessionTime, meetLink, requestedAudit }) {
  const lines = [
    `Hey ${firstName || 'there'},`,
    '',
    `You're confirmed for "The 5 AI Skills Every Business Owner Should Actually Have", ${sessionLabel || 'the next live session'}${sessionTime ? ` at ${sessionTime}` : ''}.`,
    '',
    meetLink
    ...
    '',
    'Before we go live:',
    '  • Have a real business problem in mind. You\'ll build a custom GPT around it live.',
    '  • Free accounts for Claude and ChatGPT. That\'s all you need.',
    '  • 60 minutes. Camera optional. Live Q&A at the end.',
    '',
  ];
  if (requestedAudit) {
    lines.push(
      'Heads up: you ticked the free AI audit. We\'ll reach out within 1–2 business days to schedule it, with priority scheduling for mentioning the workshop.',
      '',
    );
  }
  lines.push(
...
export async function sendConfirmationEmail({
  ...
  const payload = {
    ...
    to: [to],
    reply_to: REPLY_TO,
    subject: `You're in, ${sessionLabel || 'Nekter AI Workshop'}`,
